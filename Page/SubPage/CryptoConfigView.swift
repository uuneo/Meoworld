...
struct CryptoConfigView: View {
    ...
    var expectKeyLength:Int {
        switch manager.fields.algorithm{
    ...
    var body: some View {
        List {
            ...
            Section {
                Picker(selection: $manager.fields.padding, label: Text("Padding")) {
            ...
            Section {
                ...
                HStack{
                    ...
                    Spacer()
                    TextField(String(format: NSLocalizedString("cryptoConfigKey", comment: "输入\(expectKeyLength)位数的key"), expectKeyLength), text: $manager.fields.key)
                        .onDisappear{
                            let _ = verifyKey()
                        }
            ...
            Section {
                ...
                HStack{
                    ...
                    Spacer()
                    TextField(NSLocalizedString("cryptoConfigIv", comment: "请输入16位Iv"), text: $manager.fields.iv)
                        .onDisappear{
                            let _ = verifyIv()
                        }
                }
    ...
    func createCopyText(){
        ...
        if !verifyIv() {
            manager.fields.iv = self.randomString(length: 16)
        }
        
        if !verifyKey(){
            manager.fields.key = self.randomString(length: expectKeyLength)
        }
        ...
        let text = """
                    ...
                    def encrypt_AES_CBC(data, key, iv):
                        cipher = AES.new(key, AES.MODE_\(manager.fields.mode.uppercased()), iv)
                    ...
                    # \(String(format: NSLocalizedString("keyComment",comment: ""), Int(manager.fields.algorithm.suffix(3))! / 8))
                    key = b"\(manager.fields.key)"
                    # \(NSLocalizedString("ivComment",comment: ""))
                    iv= b"\(manager.fields.iv)"
                    
                    # 加密
                    # \(NSLocalizedString("consoleComment",comment: "")) "\((try? AESCryptoModel(cryptoFields: manager.fields).encrypt(text: "{\"body\": \"test\", \"sound\": \"birdsong\"}")) ?? "")"
